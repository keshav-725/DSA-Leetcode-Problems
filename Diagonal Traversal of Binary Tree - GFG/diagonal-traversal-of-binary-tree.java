...
class Tree
{
    int maxleft = Integer.MIN_VALUE;
     public ArrayList<Integer> diagonal(Node root)
      ...
      public void getAllDiagonal(Node root,HashMap<Integer,ArrayList<Integer>> map,int level){
          if(root==null) return;
          map.putIfAbsent(level,new ArrayList<Integer>());
          map.get(level).add(root.data);
          maxleft=Math.max(maxleft,level);
          getAllDiagonal(root.left,map,level+1);
          getAllDiagonal(root.right,map,level);
      }
}
